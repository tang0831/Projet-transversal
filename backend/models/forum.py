...
class ForumModel:
    ...
    def lister_messages(self):
        """Liste tous les messages du forum avec les infos utilisateur"""
        query = """
...
class DemandeActeModel:
    ...
    def creer_demande(self, id_utilisateur, type_acte, donnees=None):
        import json
        query = "INSERT INTO demande_acte (id_utilisateur, type_acte, donnees_complementaires) VALUES (%s, %s, %s)"
        self.conn.execute_query(query, (id_utilisateur, type_acte, json.dumps(donnees) if donnees else None))
    ...
    def lister_par_utilisateur(self, id_utilisateur):
        query = "SELECT id_demande, id_utilisateur, type_acte, statut, date_demande, donnees_complementaires FROM demande_acte WHERE id_utilisateur = %s ORDER BY date_demande DESC"
    ...
    def obtenir_demande(self, id_demande):
        query = "SELECT id_demande, id_utilisateur, type_acte, statut, date_demande, donnees_complementaires FROM demande_acte WHERE id_demande = %s"
        res = self.conn.execute_query(query, (id_demande,))
