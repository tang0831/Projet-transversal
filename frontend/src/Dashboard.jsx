...
import { getCitoyen, getActeNaissance, getActeMariage, getActeDeces, getProches, getListeActesDecesAccessibles, declarerDeces, declarerMariage } from './api';
          ...
          <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              ...
              <div className="flex gap-2">
                <a href={`http://localhost:8000/api/actes/naissance/${user?.numero_cin}/pdf`} target="_blank" rel="noopener noreferrer" className="p-2 bg-vert text-white rounded-xl hover:bg-green-700 transition"><Download size={20} /></a>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-gray-900/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden p-8">
            ...
            <form onSubmit={handleSubmitDeclaration} className="space-y-4">
              <input className="w-full px-5 py-4 bg-gray-50 rounded-2xl" placeholder="CIN Conjoint" value={declarationData.numero_cin_conjoint} onChange={(e) => setDeclarationData({...declarationData, numero_cin_conjoint: e.target.value})} required />
              <button type="submit" className="w-full py-4 bg-pink-600 text-white font-bold rounded-2xl">Envoyer la demande</button>
              <button type="button" onClick={() => setShowDeclareForm(false)} className="w-full py-4 bg-gray-100 text-gray-700 font-bold rounded-2xl">Annuler</button>
            </form>
          </div>
        </div>
      )}
